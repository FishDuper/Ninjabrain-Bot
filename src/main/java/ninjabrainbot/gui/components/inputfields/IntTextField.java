...
public class IntTextField extends JSpinner implements ThemedComponent {
	private WrappedColor bgCol;
	private WrappedColor fgCol;

	public IntTextField(StyleManager styleManager, int value, int min, int max) {
		super(new SpinnerNumberModel((double)value, (double)min, (double)max, 1.0f));
		...
		ChangeListener listener = new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				try {
					double raw = (double)((JSpinner) e.getSource()).getValue();
					int newSigma = (int)raw;
					if (newSigma > max) {
						newSigma = max;
						setValue((double)newSigma);
					} else if (newSigma < min) {
						newSigma = min;
						setValue((double)newSigma);
					}
					onChanged((double)newSigma);
				} catch (Exception ex) {
					ex.printStackTrace();
				}
			}
